...
#define DEG2RAD 3.14159/180.0	// Calculation for draw ellipse
...
GLfloat yStep = 0.1f; // Changing y coordinate value for exhaust effect
...
int carX = 95, carY = 100;	// car x and y coordinates
int carWinX = 95, carWinY = 96; // car top left and right x and y coordinates for c
...
// Game instructions to console
void gameInfo() {
	std::cout << "===Welcome to 2D CAR GAME===\n1. You can control the car using W/A/S/D keys\n";
	std::cout << "2. Press and hold R with A to rotate the car to left side\n3. Press and hold R with D to rotate the car to right side\n4. Right click on the mouse and turn on/off the grid\n";
	std::cout << "===Game Rules===\n--Avoid the obstacles and reach the green gate--\n--If car hit with a obstacle, score will reduces by 10 points--\n";
}
...
void display(void)
{
	...
	gameCheckWin(carWinX, carWinY);
	if ((gameComplete == true) || (gameScoreMax == gameScoreMinimum)) { // checking if the game has reached the green gate or the score has reached zero
...
// idle function for keyboard interaction
void idle()
{
	...
	// if d and r pressed rotation
		...
		if (keystats['a']) { // car move left
			...
			angle = angle + (90.0);
			glutPostRedisplay();
			if (carWinX + posX >= 70) // Checking whether the car is out of border or not in left
			{
				posX -= move_unit;
				for (int i = 0; i < 10; i += 2) { // Checking the car collision with a block
					if (collisionAABB(carX + posX, carY + posY, i) == true) { // checking the collision with car coordinates and square coordinates
			...
			angle = angle + (-90.0);
			glutPostRedisplay();
			if (carWinX + posX <= 720)	// Checking whether the car is out of border or not in right
			{
				posX += move_unit;
				for (int i = 0; i < 10; i += 2) {		// Checking the car collision with a block
					if (collisionAABB(carX + posX, carY + posY, i) == true) { // checking the collision with car coordinates and square coordinates
				...
				angle = 0.0;
				glutPostRedisplay();
				if (carWinY + posY <= 720) // Checking whether the car is out of border or not in top
				{
					posY += move_unit;
					for (int i = 0; i < 10; i += 2) {		// Checking the car collision with a block
						if (collisionAABB(carX + posX, carY + posY, i) == true) { // checking the collision with car coordinates and square coordinates
			...
			angle = -180.0;
			glutPostRedisplay();
			if (carWinY + posY >= 70) // Checking whether the car is out of border or not in bottom
			{
				posY -= move_unit;
				for (int i = 0; i < 10; i += 2) {			// Checking the car collision with a block
					if (collisionAABB(carX + posX, carY + posY, i) == true) { // checking the collision with car coordinates and square coordinates
...
// Save the pressed keyboard keys
...
// Save the non-pressed keyboard keys
...
int main(int argc, char** argv)
{
	...
	/* window upper left corner at (380, 60) */
	glutInitWindowPosition(380, 60);
	/* creates an OpenGL window with command argument in its title bar */
	glutCreateWindow("2D Car Game"); // window name
	init(); // calling the init function
	glutDisplayFunc(display); // calling the display function
	glutReshapeFunc(reshape); // calling the reshape funciton
	randomObstacles();	// calling the random Obstacle generator
	gameInfo(); // console output for game information
	glutIdleFunc(idle); // calling the idle function
	glutKeyboardUpFunc(keyUp); 
	glutKeyboardFunc(keyPressed);
	glutCreateMenu(glutMenu); // Creating the glut menu
	glutAddMenuEntry("Grid on", 1);	// Adding a option to turn on the grid
	glutAddMenuEntry("Grid off", 2); // Adding a option to turn off the grid
	glutAttachMenu(GLUT_RIGHT_BUTTON); // Adding the mouse button to glut pop-up menu
	glutTimerFunc(40, timerFunction, 0); // calling the timer function for animation
	glutMainLoop();
	return 0;
}

...
double bloackCoordinatesUp[10]; // Save the top upper left coordinates of the blocks
double move_unit = 0.3; // Move the car
double back_unit = 0.8;
...
void drawSquare1(GLfloat length, GLfloat centerX, GLfloat centerY) {
...
bool checkCollision1(double centerX1, double centerY1, double centerX2, double centerY2) {
	// Outputing the collision status
	bool collisionStatus;
	// Boundary values for the first sqaure
	double xMin1 = centerX1 - 30;
	double yMin1 = centerY1 - 28;
	double xMax1 = centerX1 + 30;
	double yMax1 = centerY1 + 28;
	// Boundary value for the second square
	double xMin2 = centerX2 - 35;
	double yMin2 = centerY2 - 35;
	double xMax2 = centerX2 + 35;
	double yMax2 = centerY2 + 35;
...
// Save the block top-upper-left coordinates
void blockCoordinates() {
	for (int i = 0; i < 10; i += 2) {
		bloackCoordinatesUp[i] = sortedPositionArray[i] - 35;
		bloackCoordinatesUp[i + 1] = sortedPositionArray[i + 1] + 35;
	}
}
...
/*void drawBlock() {
	...
	glBegin(GL_LINE_LOOP);
	glColor3f(red, 0, 0);
	glVertex2f(bloackCoordinatesUp[0], bloackCoordinatesUp[1]);
	glColor3f(0, green, 0);
	glVertex2f(bloackCoordinatesUp[2], bloackCoordinatesUp[3]);
	glColor3f(0, 0, blue);
	glVertex2f(bloackCoordinatesUp[4], bloackCoordinatesUp[5]);
	glColor3f(red, green, 0);
	glVertex2f(bloackCoordinatesUp[6], bloackCoordinatesUp[7]);
	glColor3f(red, 0, blue);
	glVertex2f(bloackCoordinatesUp[8], bloackCoordinatesUp[9]);
	glColor3f(red, green, blue);
	glEnd();
}*/
...
bool carBlockCollision(GLint carX ,GLint carY,double possX, double possY) {
	bool collisionDetected = false;
	for (int i = 0; i < 10; i += 2) {
		if (collisionAABB(carX + possX, carY + possY, i) == true) {
...
void display(void)
{
	...
	glTranslatef(-95, -100, 0);
	//drawSquare1(56, 95, 96);
	drawCar();
	glPopMatrix();
	//std::cout << collisionAABB(95 + posX, 96 + posY, 0) << "\n";
	//std::cout << carBlockCollision(95, 96, posX, posY);
	//carBlockCollision(95,96,posX,posY);
	//----------------car-------------------------------
...
void idle()
{
	if (keystats['r'] && keystats['a'])
	{
		...
		if (keystats['a']) {
			...
			posX -= move_unit;
			for (int i = 0; i < 10; i += 2) {
				if (collisionAABB(95 + posX, 96 + posY, i) == true) {
					std::cout << "collison\n";
					posX += 20.0f;
				}
			}
